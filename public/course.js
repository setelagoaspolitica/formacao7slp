...
$("#quiz-form").addEventListener("submit", async (event) => { event.preventDefault(); const answers = Object.fromEntries(new FormData(event.target).entries()); const correct = {q1:"b",q2:"a",q3:"c"}; const hits = Object.keys(correct).filter((key) => answers[key] === correct[key]).length; await set(ref(db, `answers/${user.uid}/checkpoint`), {answers, hits, submittedAt:Date.now()}); if (hits < 2) { $("#quiz-feedback").textContent = `Resultado: ${hits} de 3. Revise os módulos e alcance pelo menos 2 acertos para concluir.`; return; } await set(ref(db, `progress/${user.uid}/quiz`), {completedAt:Date.now(), hits}); $("#quiz-feedback").textContent = `Resultado: ${hits} de 3. Questionário concluído: +30 XP.`; showCertificate(); });
